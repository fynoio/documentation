...
![](/images/campaign-manager-role-ss.png)
### Roles & Permissions

| Feature / Access | Owner | Super Admin | Workspace Admin | Developer | Campaign Manager | Support | Default Role |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Dashboard and Analytics | Yes | Yes | Yes | Yes | Yes | Yes | No |
| Alarms and Anomalies | Yes | Yes | Yes | Yes | Yes | Yes | No |
| Integrations | Yes | Yes | Yes | Yes | View only | View only | No |
| Templates | Yes | Yes | Yes | Yes | View only | View only | No |
| Components | Yes | Yes | Yes | Yes | View only | View only | No |
| Routes | Yes | Yes | Yes | Yes | View only | View only | No |
| Notification Events | Yes | Yes | Yes | Yes | View only | View only | No |
| Logs | Yes | Yes | Yes | Test only | Yes | Yes | No |
| Report Download | Yes | Yes | Yes | Test only | Yes | No | No |
| Campaigns | Yes | Yes | Yes | Yes | Yes | View Only | No |
| User Profiles | Yes | Yes | Yes | Test only | View only | View only | No |
| User Cohorts | Yes | Yes | Yes | No | View only | View only | No |
| User Preferences | Yes | Yes | Yes | Yes | View only | View only | No |
| Invite Team Member | Yes | Yes | Only with "Invite Team Members and Modify Roles" add-on policy | Only with "Invite Team Members and Modify Roles" add-on policy | View Only | View Only | View Only |
| API Keys | Yes | Yes | Yes | Yes | View only | View only | No |
| Allowlist (Webhooks) | Yes | Yes | Yes | Yes | View only | View only | No |
| Security Settings | Yes | Yes | View only | View only | View only | View only | No |
| Delivery Limits (DND) | Yes | Yes | Yes | Yes | View only | View only | No |
| Add-ons | Yes | Yes - can configure all add-on policies including Approver and Team Management permissions | Yes - can assign add-on policies based on granted permissions | Yes - can assign add-on policies based on granted permissions | View only | View only | No |
| Copy to another Workspace | Yes | Yes | Yes | No | No | No | No |
| Workspace Creation / Deletion | Yes | No | No | No | No | No | No |

...
![](/images/invite-team-members-role.png)
### Roles & Permissions
...
<style>{`
  ...
  /* Scoped, paired background + text overrides for THIS table only.
     Fixes a bug where an unscoped "tr td { background !important }" rule
     was painting every table on the page black in dark mode without
     ever forcing a matching text color, making body rows unreadable. */
  .fyno-roles-table-wrapper table.fyno-roles-table td {
    background-color: var(--rb-table-bg, #FFFFFF) !important;
    color: var(--rb-table-text, #1a1a1a) !important;
  }
  .fyno-roles-table-wrapper table.fyno-roles-table th {
    background-color: var(--rb-header-bg, #F0F0F0) !important;
    color: var(--rb-header-text, #333333) !important;
  }
  .fyno-roles-table-wrapper table.fyno-roles-table tr td[colspan] {
    background-color: var(--rb-category-bg, #FAF4E3) !important;
    color: var(--rb-category-text, #8B6602) !important;
  }
...
<div style={tableWrapperStyle} className="fyno-roles-table-wrapper">
  <table style={tableStyle} className="fyno-roles-table">
    <thead>
      <tr>
...
| Add-On Policy | What it allows |
| --- | --- |
| Report Download | Download both test and live reports. |
| Unmask Sensitive Data | Unmask masked fields in logs and download reports containing unmasked data. |
| Invite team members and modify roles | Invite, remove, and modify roles and add-on policies of team members. |
| Approver | View and approve modules that have approval enabled. |
...
To see when masked data was unmasked and by whom, navigate to **Workspace Settings** > **Security** and click **View Unmasking Activity** in the **Data Masking** section. For more information, see [View Unmasking Activity](https://www.fyno.io/docs/security/data-masking#view-unmasking-activity).

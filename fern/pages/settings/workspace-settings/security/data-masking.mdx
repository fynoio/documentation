...
- To give other users access to unmasked data, assign them the [Unmask Sensitive Data](https://www.fyno.io/docs/team#unmask-sensitive-data) add-on policy.
...
### View Unmasking Activity

Unmasking Activity shows when masked data was unmasked and who unmasked it, so you can keep track of who has viewed sensitive information.

<Steps>
  <Step>Navigate to **Workspace Settings** > **Security**.</Step>
  <Step>In the **Data Masking** section, click **View Unmasking Activity**.</Step>
</Steps>

Activities are listed with the most recent first. Each entry shows:

| Detail | What it shows |
| --- | --- |
| **Date and time** | When the data was unmasked. |
| **Team member** | Who unmasked the data. |
| **Activity** | Where the data was unmasked, for example, **Unmasked Sent Logs**. |
| **Date range** | The date range of the logs being viewed when the data was unmasked. |
| **Rows and page** | How many rows were unmasked and on which page, for example, **30 rows of Page 1**. |

To learn who can unmask data in logs and how, see [Data Masking in logs](https://www.fyno.io/docs/logs-overview#data-masking-in-logs).
...
![](/images/default-masking-view.png)
When enabled:

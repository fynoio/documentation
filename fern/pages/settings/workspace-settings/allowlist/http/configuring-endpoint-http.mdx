...
The **Configure Endpoint** section allows you to define how delivery reports are received, how request parameters are passed, how encryption is handled, and how requests are authenticated for the endpoint.
...
> If your endpoint requires an OAuth access token, use the [OAuth](#oauth) section instead of adding the token manually as a header.
...
<info> Refer to the [Secrets](./security) documentation to learn how to create and manage reusable secure keys. The keys displayed in the UI are examples and may differ based on your workspace configuration.</info>
...
The **OAuth** section allows you to authenticate delivery report requests using an OAuth access token. Use this when the configured endpoint only accepts requests that carry a bearer token obtained through OAuth.

Instead of generating and passing the token yourself, you select a saved OAuth configuration, and Fyno obtains the token and includes it with every request sent to the endpoint.

![](/images/configure-endpoint-oauth.png)

### Enable OAuth

1. Select the **OAuth** tab in the **Configure Endpoint** section.
2. Turn on the **API endpoint requires OAuth token** toggle.
3. From the **OAuth Configuration** dropdown, select the OAuth configuration to use for this endpoint.

![](/images/configure-endpoint-oauth-select.png)

If you haven't created an OAuth configuration yet, the dropdown shows **No options**. Select **+ Add Configuration** to create one.

#### How It Works
1. Before sending a request to the endpoint, Fyno requests an access token from the **Token URL** defined in the selected OAuth configuration.
2. Fyno sends the request to the endpoint with the access token included as a bearer token.
3. The endpoint validates the token and processes the request.

### OAuth Fields

| Field | Description |
| :--- | :--- |
| **API endpoint requires OAuth token** | Enables OAuth authentication for requests sent to this endpoint. |
| **OAuth Configuration** | The saved OAuth configuration used to obtain the access token. |

> Note: OAuth configurations are created and managed under **Workspace Settings** > **Security** > **OAuth Configuration**. Currently, only the **Client Credentials** grant type is supported.

#### To Know More
An OAuth configuration stores the Token URL, Client ID, Client Secret, and any optional Scope, Audience, or custom headers required by your OAuth provider. The Client ID and Client Secret are selected from [Secrets](./security), and a single configuration can be reused across multiple allowlisted endpoints that share the same credentials.

> Refer to the [OAuth Configuration](./security/oauth-configuration) documentation to learn how to create, test, and manage OAuth configurations.
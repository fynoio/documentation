...
| msg_content          | **JSON Object**; Contains information of the template sent.<br/>For SMS, it will include the SMS text and template ID (if available).<br/>For WhatsApp, it will include the template_name, language, and placeholder mapping.<br/>**Note:** This variable is enabled on request and is available only for SMS, WhatsApp. If enabled, only the first webhook call will contain this variable. |

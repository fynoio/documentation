...
- **WhatsApp Templates**
  ...
  - **Support for mixed button combinations:** Updated the WhatsApp external template builder to align with Meta’s latest guidelines, allowing mixed button combinations such as Quick Replies with Call-to-Action or Flow buttons in the same template.
  - Validation rules now exactly mirror Meta’s supported combinations, ensuring Meta-approved templates can be recreated accurately in Fyno and preventing valid templates from being blocked or misconfigured.
...
- **Campaigns**
  ...
  - **Accurate failed message count in Campaign Details banner:** Fixed an issue where the Campaign Details banner always displayed 0 failed messages. The banner now correctly reflects the failed message count from `campaignDetails.meta_data.p_count.submit_failed`.
...
- **Notifications**
  ...
  - **Fixed trigger disablement for unlimited limits:** Resolved an issue where notification and campaign triggers were disabled when `notifications_upper_limit` was null. A null value is now correctly treated as unlimited.

- **User Suppression**

  - **Fixed UI crash caused by race condition:** Resolved an intermittent crash caused by accessing `reasonsByChannel` before the API response was available. The UI now safely handles loading and undefined states.

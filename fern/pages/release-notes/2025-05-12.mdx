...
- **Enhanced Callback Management** – Retry support for failed requests.
- **Integration Updates** – Support for MM LITE in WhatsApp sign-up, Meta session logging, and AWS Pinpoint SMS integrations (Beta).
- **PDF Support** – Components can now generate PDFs.
- **IP Allowlisting** – Incoming messages now support IP-based allowlisting.

### ⚡ Feature Enhancements

- **Workflows** – Added the option to configure multiple action-condition blocks for workflows using the Meta trigger; simplified the HTTP widget.
- **Alarms** – Added support for campaign-based alarms, updated the alarm status list, and now display logs when alarm severity is reduced.
...
- **Integrations** – Added a session logging event listener for Meta.

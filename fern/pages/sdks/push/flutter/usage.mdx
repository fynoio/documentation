---
title: 'Runtime API Reference: Flutter Push SDK'
slug: 'flutter-sdk-usage-guide'
description: 'Initialize lifecycle pipelines, bind unique distinct identity passports, manage background multi-tenant routing, and orchestrate device token tracking variables using Dart.'
og:title: 'Runtime API Reference: Flutter Push SDK | Fyno Documentation'
og:description: 'Initialize lifecycle pipelines, bind unique distinct identity passports, manage background multi-tenant routing, and orchestrate device token tracking variables using Dart.'
...
For Google FCM, ensure you've followed the Firebase setup [mentioned earlier](./flutter-prerequisites-advanced-setup-requirement) and have added the `google-services.json` or `GoogleService-Info.plist` file to your project root.

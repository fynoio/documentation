...
To create a workflow, you need to choose a trigger. This is based on how you want to send payload to workflow.

- Webhooks - Create the Workflow using a Webhook as the trigger.
- Virtual Mobile No - A virtual mobile number with the capability to receive incoming messages can be configured here . Contact [support@fyno.io](mailto:support@fyno.io) for configuring a virtual number.
...
<Steps>
  <Step>In the Workflow, configure the WABA no by adding 'Virtual No' trigger. ![](/images/966ff8a-virtual_no.png)</Step>
  <Step>Drag and drop a Condition block and configure it as shown below. Its critical to use the variable '**$input.body.simplified.message.text**' and value as '**Stop promotions**' ![](/images/b386c56-subs_if.png)</Step>
  <Step>Drag and drop 'Subscription' action and attach it to if block. Then add Subscription key as 'simplified.message.button.payload' and action as 'Opted-Out'. ![](/images/9d34db2-sub_key.png)</Step>
</Steps>
...
- **Operator:** Select an operator that you want to define the condition with. It can be any of the following:
  ...
  - In
  - Greater Than
  - Less Than8. Greater than or Equal
  - Less than or Equal
  - Is

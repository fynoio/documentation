---
title: 'Tracing Traffic with Notification Event Logs'
slug: 'notification-event-logs'
description: 'Analyze programmatic handshakes, message execution trails, and channel routing logic. Learn how to debug failover cascades, isolate runtime exceptions, and utilize search parameters.'
og:title: 'Tracing Traffic with Notification Event Logs | Fyno Documentation'
og:description: 'Analyze programmatic handshakes, message execution trails, and channel routing logic. Learn how to debug failover cascades, isolate runtime exceptions, and utilize search parameters.'
...
- **Failover Component**: This component shows the channels that were selected as failovers to the primary channel (the "[Channel Component](./omnichannel-route-creation#channels)" preceding it ), along with the details of the failover attempted.  
   This component will only be displayed if:
  - The "Routing" in the Notification Event is an "Omnichannel Route",
...
- **Wait Component**: When configuring an Omnichannel route, if a "[Wait for](./omnichannel-route-creation#controls)" component was configured, the same will be displayed in the notification event logs.
...
![](/images/logs_notification_event.png)
...
- **Trace**: Gives you a summary of each hop or path that the notification has taken along with the time that each hop has taken, including the total as well as the final status of the notification. Read more [here](./log-details-trace).
- **Summary**: Gives you a summary of the notification. Read more [here](./log-details-summary).
- **Placeholders**: Tells you the details of the [Placeholders](./log-details-placeholders) used in the notification template.
- **Content**: Displays the details of the content triggered in the notification. [Read more](./log-details-content).
- **Response**: Shows all the details of the notification response that was received from the Provider. Understand more about [Response](./log-details-response).
- **Delivery**: Provides you a detailed account of the delivery response received from the provider. [Read more](./log-details-delivery).
...
"Timestamp" allows you to specify the time frame for which you want to view the logs.

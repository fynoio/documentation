---
title: 'How to Receive Delivery Reports (DLR)'
slug: 'how-to-receive-delivery-reports'
description: 'Learn how to configure allowlisted endpoints and receive notification delivery reports (DLRs) from Fyno through APIs, Campaigns, and Workflows.'
og:title: 'Receive Delivery Reports (DLR) | Fyno Documentation'
og:description: 'Configure allowlisted URLs or SQS endpoints, customize delivery callback payloads, and receive notification delivery status updates from Fyno.'
...
<Steps>
  <Step>Select the 'Configure as Delivery Endpoint' by clicking the ellipsis as shown. <img src="/images/allowlist_configure_delivery_endpoint.png" width="500px" height="auto" style="padding:1px;border:thin solid black"/> </Step>
...
![](/images/configure-delivery-endpoint.png)
...
>
> **What changed:**
> Earlier, custom fields were only available when configuring delivery reports, which made it mandatory to set up an Allowlist URL. Now, custom fields are available in a separate section in the UI and can be configured without enabling delivery callbacks.
>
> **What this means:**
...
| metabtnreply | JSON Object | Contains information about the button reply received in response to a WhatsApp message.<br/>**Note:** Applicable only to WhatsApp notifications. |
| analytics_label | String | The analytics label associated with the Notification Event that is being triggered. |
| credits_used | String | The number of credits consumed for the template that was sent.<br/>**Note:** Applicable only to SMS notifications. |
| grouping_name | String | The group label associated with the Notification Event that is being triggered. |
| campaign_name | String | The name of the campaign used to trigger the Notification Event.<br/>**Note:** Sent only when the event is triggered through a campaign. |

<Info>Some of the default parameters are channel- or trigger-specific and will not appear in every delivery report: `metabtnreply` is sent only for WhatsApp button replies, `credits_used` only for SMS, and `campaign_name` only when the notification event is triggered through a campaign.</Info>
...
![](/images/campaign-delivery-report-endpoint-updated.png)
...
<Steps>
  ...
  <Step>It opens the page as shown below. Select the 'Do you want to receive a delivery report?' checkbox. ![](/images/create-campaigns-custom.png)</Step>
  <Step>
    ...
    <ol>
      ...
      <li>You can also configure custom parameters to be sent along with delivery reports. The supported fields are: custom_id, custom1, custom2, and custom3. These can be passed via the workflow payload or mapped using variables, and will be included in the delivery callback for downstream tracking and reporting.</li>
    </ol>
  </Step>
</Steps>
...
<ParamField path="Custom Field 3" type="string" required={false}>
  <ul>
    <li>Additional information to tag your notification</li>
    <li>**Length**: 50 characters</li>
    <li>**Example:** `message-id`</li>
...
```json title="Sample Callback Response" wordWrap
{
  ...
  "channel": "email",
  "analytics_label": "YOUR_ANALYTICS_LABEL",
